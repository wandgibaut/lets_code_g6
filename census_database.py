...
class Database():
    ...
    def _list_files(self):
        # check files in local dir
        file_list = sorted(
            [i for i in os.listdir('input_files') if i[-3:] == 'csv'])
        return file_list
    ...
    def _readmerge_files(self):
        ...
        for file in self.file_list:
            ...
            file_content = pd.read_csv(
                file_path, delimiter=';')  # read file content
            ...
            if not are_commun_columns:
                # If not = concat by columns
                data = pd.concat([data, file_content], axis=1)
            else:
                data = data.merge(file_content, on=list(
                    common_columns))  # If does = merge on those
